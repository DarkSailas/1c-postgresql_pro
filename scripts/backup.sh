#! /bin/bash
export POSTGRES_HOST= #тут нужен айпи хоста с контейнером postgres
export POSTGRES_DB= #через пробел или запятую базы для бэкапа
export POSTGRES_USER= #юзер из переменных
export POSTGRES_PASSWORD= #его пасс
...
export POSTGRES_PASSFILE_STORE=**None**
export BACKUP_DIR= #каталог бэкапов, берется из монтированых каталогов из docker-compose.yml
